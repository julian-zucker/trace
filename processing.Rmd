...
library(ggplot2)
library(stringr)
evals <- read.csv("TRACE Evaluations.csv")


# Read all the files into a list
files <- list.files(".", full.names = TRUE)
files <- files[which(str_detect(files, "eval"))]
file <- files[2]
evalsDF <- t(data.frame(fieldNames))

for (file in files) {
  load(file)
  
  evals <- evals[-1:-2]
  evals <- evals[which(sapply(evals, Negate(is.null)))]
